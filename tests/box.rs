...
const MSG: [u8; 104] = [
    ...
    0x16, 0x17, 0x18, 0x19, 0x20, 0x21, 0x22, 0x23, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
...
const SK2: [u8; 32] = [
    0x85, 0xd6, 0xbe, 0x78, 0x57, 0x55, 0x6d, 0x33, 0x7f, 0x44, 0x52, 0xfe, 0x42, 0xd5, 0x06, 0xa8,
    0x01, 0x03, 0x80, 0x8a, 0xfb, 0x0d, 0xb2, 0xfd, 0x4a, 0xbf, 0xf6, 0xaf, 0x41, 0x49, 0xf5, 0x1c,
];
...
fn test_secretbox() {
    ...
    let r = secret::key(&KEY).nonce(&NONCE).open(&mut pt, &ct, &tag);
    assert!(r);
    assert_eq!(&pt[..], &MSG[..]);
}
...
fn test_sealedbox() {
    ...
    sealed::SecretKey(SK1)
        .and(&pk2)
        .nonce(&NONCE)
        .seal(&MSG, &mut ct, &mut tag);

    let r = sealed::SecretKey(SK2)
        .and(&pk1)
        .nonce(&NONCE)
        .open(&mut pt, &ct, &tag);
    assert!(r);
